...
{
    internal class csshprpg_database
    {
...
#define RECONNECT_INTERVAL 360.0
        ...
        {
            // native bool SMRPG_ResetAllPlayers(char[] sReason, bool bHardReset=false);
...
void FailDatabaseUpdateError(int iVersion, char[] sQuery)
{
    char sError[256];
    SQL_GetError(g_hDatabase, sError, sizeof(sError));
...
public void SQL_DoNothing(Database db, DBResultSet results, char[] error, any data)
{
    if (results == null)
    {
...
public void SQLTxn_LogFailure(Database db, any data, int numQueries, char[] error, int failIndex, any[] queryData)
{
    LogError("Error executing query %d of %d queries: %s", failIndex, numQueries, error);
}

public void SQL_GetUpgradeInfo(Database db, DBResultSet results, char[] error, any index)
{
    if (results == null)
    {
...
public void SQL_InsertNewUpgrade(Database db, DBResultSet results, char[] error, any index)
{
    if (results == null)
    {
...
public void SQL_DeleteExpiredPlayers(Database db, DBResultSet results, char[] error, any data)
{
    if (results == null)
    {
 ...
 */
stock bool SQL_LockedFastQuery(Database database, char[] query, int len = -1)
{
