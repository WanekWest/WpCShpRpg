...
{
    internal class csshprpg_stats
    {
        // Allow to refetch the rank every 20 seconds.
        int RANK_CACHE_UPDATE_INTERVAL = 20;

        int[] g_iCachedRank = new int[Server.MaxPlayers + 1];
        int[] g_iNextCacheUpdate = new int[Server.MaxPlayers + 1];
        int g_iCachedRankCount = 0;
        int g_iNextCacheCountUpdate;

        public csshprpg_stats()
        {

        }

        private uint g_hCVBotMaxlevel, g_hCVPlayerMaxlevel, g_hCVPlayerMaxlevelReset
            , g_hCVExpStart, g_hCVExpInc, g_hCVExpMax, iMaxLevel;

        private bool bMaxLevelReset, g_hCVBotMaxlevelReset;

        public csshprpg_stats(uint hCVBotMaxlevel, bool hCVBotMaxlevelReset, uint hCVPlayerMaxlevel, uint hCVPlayerMaxlevelReset
            , uint hCVExpStart, uint hCVExpInc, uint hCVExpMax, uint MaxLevel, bool MaxLevelReset)
        {
            g_hCVBotMaxlevel = hCVBotMaxlevel;
            g_hCVBotMaxlevelReset = hCVBotMaxlevelReset;
            g_hCVPlayerMaxlevel = hCVPlayerMaxlevel;
            g_hCVPlayerMaxlevelReset = hCVPlayerMaxlevelReset;
            g_hCVExpStart = hCVExpStart;
            g_hCVExpInc = hCVExpInc;
            g_hCVExpMax = hCVExpMax;
            iMaxLevel = MaxLevel;
            bMaxLevelReset = MaxLevelReset;
        }
        ...
        SessionStats[] g_iPlayerSessionStartStats = new SessionStats[Server.MaxPlayers + 1];
        bool[] g_bBackToStatsMenu = new bool[Server.MaxPlayers + 1];
        ...
        AFKInfo[] g_PlayerAFKInfo = new AFKInfo[Server.MaxPlayers + 1];
        bool[] g_bPlayerSpawnProtected = new bool[Server.MaxPlayers + 1];

        // StringMap g_hWeaponExperience;

        struct WeaponExperienceContainer
        {
            float damage;
            float kill;
            float bonus;
        ...
        uint Stats_LvlToExp(uint iLevel)
        {
            uint iExp;
            ...
            if (iLevel <= 1)
                ...
                iExp = iLevel * g_hCVExpInc + g_hCVExpStart;

            return iExp > g_hCVExpMax ? g_hCVExpMax : iExp;
        }

        /* Calculate how many levels to increase by current level and experience */
        uint Stats_CalcLvlInc(uint iLevel, uint iExp)
        {
            uint iLevelIncrease = 0;
            uint iExpRequired = Stats_LvlToExp(iLevel);
        ...
        void Stats_PlayerNewLevel(int client, int iLevelIncrease)
        {
            ...
            {
                iMaxLevel = g_hCVBotMaxlevel;
                bMaxLevelReset = g_hCVBotMaxlevelReset;
            }
            else
            {
                iMaxLevel = g_hCVPlayerMaxlevel;
                bMaxLevelReset = g_hCVPlayerMaxlevelReset;
            }
            ...
            if (iMaxLevel > 0)
            {
                ...
                if (iNewLevel > iMaxLevel)
                {
                    ...
                    if (bMaxLevelReset)
                    {
                        Console.WriteLine("Player %N has surpassed the maximum level of %d, resetting his stats", client, iMaxLevel);
                        Server.PrintToChatAll($"{client} reached maxlevel {iMaxLevel}");
                        Console.WriteLine("%L surpassed the maximum level of %d, resetting his stats.", client, iMaxLevel);
                        ResetStats(client);
                        return;
                    }
                    else
            ...
            Console.WriteLine("%N is now level %d (%d level increase(s))", client, GetClientLevel(client), iLevelIncrease);
        ...
        bool Stats_AddExperience(int client, int iExperience, char[] sReason, bool bHideNotice, int other, bool bIgnoreChecks = false)
        {
            // Nothing to add?
            if (iExperience <= 0)
                return false;
        ...
        void Stats_PlayerDamage(int attacker, int victim, float fDamage, char[] sWeapon)
        {
        ...
        void Stats_PlayerKill(int attacker, int victim, char[] sWeapon)
        {
            // Ignore suicide.
            if (attacker == victim)
                return;
        ...
        // forward Action SMRPG_OnAddExperience(int client, char[] reason, int iExperience, int other);
        Action Stats_CallOnExperienceForward(int client, char[] sReason, int iExperience, int other)
        {
            Action result;
        ...
        // forward void SMRPG_OnAddExperiencePost(int client, char[] reason, int iExperience, int other);
        void Stats_CallOnExperiencePostForward(int client, char[] sReason, int iExperience, int other)
        {
         ...
         * Native Callbacks
         */
        // native bool SMRPG_AddClientExperience(int client, int &exp, char[] reason, bool bHideNotice, int other=-1, SMRPG_ExpTranslationCb callback=INVALID_FUNCTION);
        public int Native_AddClientExperience(Handle plugin, int numParams)
        {
            ...
            if (bAdded && !IsFakeClient(client))
            {
                ...
                {
                    // functag SMRPG_ExpTranslationCb(client, char[] reason, iExperience, other, char[] buffer, maxlen);
        ...
        public void SQL_GetTop10Native(Database db, DBResultSet results, char[] error, DataPack data)
        {
            data.Reset();
        ...
        // native float SMRPG_GetWeaponExperience(char[] sWeapon, WeaponExperienceType type);
        ...
        void InsertSessionExperienceString(int client, char[] sExperience)
        {
        ...
        public void ConVar_LastExperienceCountChanged(ConVar convar, char[] oldValue, char[] newValue)
        {
        ...
        public void SQL_GetClientRank(Database db, DBResultSet results, char[] error, any userid)
        {
        ...
        public void SQL_GetRankCount(Database db, DBResultSet results, char[] error, any data)
        {
            if (results == null)
            {
        ...
        public void SQL_GetTop10(Database db, DBResultSet results, char[] error, any userid)
        {
            int client = GetClientOfUserId(userid);
            if (!client)
                return;
        ...
        public void SQL_GetNext10(Database db, DBResultSet results, char[] error, any userid)
        {
            int client = GetClientOfUserId(userid);
            if (!client)
                return;
    ...
    float GetWeaponExperience(char[] sWeapon, WeaponExperienceType type)
    {
        WeaponExperienceContainer weaponExperience;
    ...
    stock void RemovePrefixFromString(char[] sPrefix, char[] sInput, char[] sOutput, int maxlen)
    {
