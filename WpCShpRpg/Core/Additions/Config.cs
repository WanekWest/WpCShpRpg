...
namespace WpCShpRpg
{
    public class ConfiguraionFiles
    {
        ...
        public bool LoadModCondiguration(string ModuleDirectory)
        {
            ...
            {
                Server.PrintToConsole($"Ошибка: Не удалось найти файл по пути {ParentDirectory}!");
                return false;
        ...
        public void LoadExecutionFile(string ModuleDirectory)
        {
            ...
            string configPath = Path.Combine(ParentDirectory, "configs/wpcshrpg/wpcshprpg_execute.cfg");
            if (!File.Exists(configPath))
            {
                Server.PrintToConsole("Error: Unable to find the parent directory for the module.");
                return;
